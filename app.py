...
def load_data():
    ...
    try:
        # [수정됨] dtype=str을 추가하여 DtypeWarning을 방지합니다.
...
def find_drug_info(df, query):
    ...
    try:
        ...
        if search_results.empty:
            return None # [수정됨] find_drug_info의 반환값을 일관되게 None으로 변경
...
def check_drug_interaction_flexible(df, drug_A_query, drug_B_query):
    """ [수정됨] 부분 검색어를 사용해 상호작용을 직접 검색하는 로직 """
    
    # 1. 사용자가 입력한 부분 검색어를 준비합니다. (숫자, 한글 등 모두 포함)
    pattern_A = re.escape(drug_A_query)
    pattern_B = re.escape(drug_B_query)

    # 2. 약물 A와 B가 포함된 모든 컬럼을 정의합니다.
    cols_A = (df['제품명A'].str.contains(pattern_A, na=False, case=False) | df['성분명A'].str.contains(pattern_A, na=False, case=False))
    cols_B = (df['제품명B'].str.contains(pattern_B, na=False, case=False) | df['성분명B'].str.contains(pattern_B, na=False, case=False))

    # 3. 약물 B와 A가 포함된 모든 컬럼을 정의합니다. (순서 반대)
    cols_C = (df['제품명A'].str.contains(pattern_B, na=False, case=False) | df['성분명A'].str.contains(pattern_B, na=False, case=False))
    cols_D = (df['제품명B'].str.contains(pattern_A, na=False, case=False) | df['성분명B'].str.contains(pattern_A, na=False, case=False))

    # 4. (A & B) 또는 (B & A) 조합에 해당하는 모든 상호작용을 한 번에 찾습니다.
