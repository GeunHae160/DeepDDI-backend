...
def load_data():
    ...
    try:
        if not os.path.exists(DB_FILE):
            ...
            # [수정됨] fuzzy=True 옵션을 추가하여 대용량 파일 경고를 무시하고 다운로드
        ...
        if match_component:
            ...
            if drug_name:
                ...
                if not results.empty:
                    components = set()
                    # [수정] 검색어가 '제품명A'에 있으면 '성분명A'를, '제품명B'에 있으면 '성분명B'를 선택
                    target_pattern = re.escape(drug_name)
                    for _, row in results.iterrows():
                        # A컬럼에 검색어가 포함되어 있으면 -> 성분A 추가
                        if pd.notna(row['제품명A']) and re.search(target_pattern, row['제품명A'], re.IGNORECASE):
                            if pd.notna(row['성분명A']): components.add(row['성분명A'])
                        
                        # B컬럼에 검색어가 포함되어 있으면 -> 성분B 추가
